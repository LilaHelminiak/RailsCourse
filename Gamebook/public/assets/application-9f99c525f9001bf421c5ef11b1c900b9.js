...
(function() {
  this.preview = function() {
    var answer, answer_text, answers, parse, parse_newlines, text;
    return $(text = document.getElementById('article_text').value, answers = text.match(/{(.*?)}/g), parse = text, (function() {
      var _i, _len, _results;
      _results = [];
      for (_i = 0, _len = answers.length; _i < _len; _i++) {
        answer = answers[_i];
        answer_text = answer.split(';')[0].substr(1);
        _results.push(parse = replace(parse, answer, '<input type=\"radio\">' + answer_text));
      }
      return _results;
    })(), parse_newlines = replace(parse, /\n/g, "<br/>"), document.getElementById('preview').innerHTML = parse_newlines);
  };

  this.replace = function(text, pattern, replace_msg) {
    return text.replace(pattern, replace_msg);
  };
